...
- 📌 **Закрепление приложений** — правый клик → «Закрепить» (до 6), любимцы всегда сверху в своём порядке
- 🎛 **Эквалайзер на 10 полос** — общий на всю систему плюс свой для любого приложения (свой перебивает общий), с пресетами
- 💾 **Настройки запоминаются** — приглушил один раз, останется и после перезапуска
- 🔊 **Мастер-громкость** всей системы в той же панели
- 🔄 **Сам сообщает об обновлениях** — автообновление через Sparkle, новая версия ставится в один клик
- 🪶 **Без драйверов и расширений ядра** — официальный API CoreAudio (process taps)

## 🚀 Установка

**[⬇️ Скачать DMG последнего релиза](https://github.com/djet00/VolumeMixer/releases/latest)** → перетащить в «Программы» → запустить.

Требуется macOS 15+.

<details>
<summary>⚠️ macOS скажет «разработчик не подтверждён» — что делать (один раз)</summary>
...
Клик по иконке микшера в menu bar (справа сверху, рядом с часами) — открывается панель.
...
./release.sh     # полный релиз: DMG + подпись Sparkle + appcast + GitHub Release
```

Автообновление: приложение раз в день сверяется с [appcast.xml](appcast.xml) в этом
репозитории; обновления подписаны EdDSA-ключом (Sparkle), релизы лежат в GitHub Releases.
